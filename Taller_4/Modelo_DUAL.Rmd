...
boxplots = ggplot(df_long, aes(x = Variable, y = Valor, fill = Region)) +
  ...
  theme(
    ...
    strip.text = element_text(face = "bold", hjust = 0.5)  # títulos de facetas en negrilla y centrados
  ) +
...
```{r serie, fig.width=12, fig.height=8, fig.align='center', echo=FALSE, warning=FALSE, message=FALSE}
cat("\n")
...
ggplot(df_long, aes(x = Año, y = mediana, color = Region, group = Region)) +
  geom_line(linewidth = 1) +      # Dibuja las líneas
  facet_wrap(~ variable, scales = "free_y") + # Crea un panel por cada variable
  labs(
    title = "Mediana Anual por Región y Variable",
    x = "Año",
    y = "Valor de la Mediana",
    color = "Región"
  ) +
  theme_light()
```

En la mayoría de las variables se tiene una caida en el año 2020 debido a la pandemia del COVID-19 (como las variables de ahorro, inversión extranjera, o Exportaciones/Importaciones), y otras suben (como los gastos). Luego, se presentan recuperaciones a niveles antes de la pandemia, o incluso superiores. A continuación, se presenta un análisis detallado por año y por región.
...
kable(
  ...
  col.names = c("Categoría", "Patrones Destacados")   # 👈 ESTA ES LA LÍNEA NUEVA
) %>%
  kable_styling(
    bootstrap_options = c("striped", "hover", "condensed", "responsive"),
    full_width = FALSE,
...
analisis_dmfa_extendido = data.frame(
  ...
  Hallazgos_Clave = c(
    "Correlación POSITIVA fuerte con: Servicios, Recaudación_Impuestos, Gastos, Exportaciones, Importaciones.\nCorrelación NEGATIVA fuerte con: Agricultura.\n\nConclusión: Contrasta **economías avanzadas** (Servicios/Fiscalidad alta) vs. **economías primarias** (Agricultura).",
    "Correlación POSITIVA fuerte con: Inversión Extranjera (Entrada/Salida).\n\nConclusión: Captura el **dinamismo de los flujos de capital internacional**, diferenciando economías según su rol como receptoras/emisoras de inversión.",
    "Correlación POSITIVA fuerte con: Ahorro y Producción.\nCorrelación NEGATIVA moderada con: Servicios.\n\nConclusión: Relacionado con la **disciplina fiscal** y la capacidad de generar riqueza interna vía ahorro y producción.",
    "Correlación POSITIVA fuerte con: Remesas.\nCorrelación NEGATIVA fuerte con: Gastos Militares.\n\nConclusión: Captura factores de **ingreso social externo** (Remesas) frente a **prioridades de seguridad/estratégicas** (Gasto Militar)."
  ),
  Nombre_Propuesto = c(
    "Eje de Desarrollo y Modernización Económica",
    "Eje de Flujos de Capital Internacional",
    "Eje de Ahorro Interno y Producción",
  ...
  Significado_Implicacion = c(
    ...
    "Indica países con capacidad de generar riqueza interna vía ahorro y producción.",
...
knitr::kable(analisis_dmfa_extendido,
      format = "html",
      align = "l",
      col.names = c("Dimensión", "Hallazgos Clave", "Nombre Propuesto", "Significado e Implicación", "Varianza Explicada"),
...
analisis_vars_2019 = data.frame(
  ...
  Proyeccion_Fuerza = c(
    ...
    "Correlación alta en Dim. 1, baja en Dim. 2"
  ),
  Hallazgo_Clave = c(
...
analisis_vars_2020 = data.frame(
  ...
  Hallazgo_Clave = c(
    "El factor fundamental de **Desarrollo vs. Primario** se mantiene inmutable a pesar del shock por la pandemia, confirmando la estabilidad de la estructura económica base.",
...
knitr::kable(analisis_vars_2022,
      format = "html",
      align = "l",
      col.names = c("Eje/Patrón", "Variables Clave", "Cambio vs. 2021 (Post-Crisis)", "Hallazgo Clave (Contexto 2022)"),
